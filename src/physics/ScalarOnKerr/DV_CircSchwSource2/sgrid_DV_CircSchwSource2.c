...
int sgrid_DV_CircSchwSource2(void) 
{
  ...
  AddPar("DV_CircSchwSource_DVWindow_smooth", "0",
         "if smooth=3 source will be artificially smoothed to C^3");
  AddPar("DV_CircSchwSource_DVWindow_q1", "1.2", "q1 determines the value of r where the window function, W=1/2");
  AddPar("DV_CircSchwSource_DVWindow_s1", "1.9", "slope of W");
  AddPar("DV_CircSchwSource_DVWindow_r1", "0", "r1 is the inner radius where W=0");
  AddPar("DV_CircSchwSource_DVWindow_r2", "0", "r2 is the outer radius where W=1");
  AddPar("DV_CircSchwSource_DVWindow_q3", "0", "q3 determines the value of r where the window function, W=1/2");
  AddPar("DV_CircSchwSource_DVWindow_s3", "0", "slope of W");
  AddPar("DV_CircSchwSource_DVWindow_r3", "0", "r3 is the inner radius where W=1");
  AddPar("DV_CircSchwSource_DVWindow_r4", "0", "r4 is where W=0");
